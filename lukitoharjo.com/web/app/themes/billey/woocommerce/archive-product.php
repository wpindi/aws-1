					...
					<?php if ( have_posts() ) : ?>
						...
						<?php if ( ! is_shop() || $shop_page_display !== 'subcategories' ) : ?>
							...
							<?php if ( wc_get_loop_prop( 'total' ) ) { ?>
								...
								$grid_class = 'billey-grid';
								$lg_columns = Billey::setting( 'shop_archive_lg_columns' );
								$md_columns = Billey::setting( 'shop_archive_md_columns' );
								$sm_columns = Billey::setting( 'shop_archive_sm_columns' );

								if ( 'none' === Billey_Global::instance()->get_sidebar_status() ) {
									$lg_columns = '3';
								}
